...
const skillGroups = [
  { label: 'LANGUAGES', skills: [{ name: 'JavaScript', icon: SiJavascript }, { name: 'TypeScript', icon: SiTypescript }, { name: 'Python', icon: SiPython }] },
  { label: 'FRAMEWORKS', skills: [{ name: 'React', icon: SiReact }, { name: 'Next.js', icon: SiNextdotjs }, { name: 'Node.js', icon: SiNodedotjs }, { name: 'FastAPI', icon: SiFastapi }] },
  { label: 'TOOLS / AI', skills: [{ name: 'REST API', icon: Braces }, { name: 'SQL', icon: Database }, { name: 'RAG', icon: BrainCircuit }, { name: 'Git', icon: SiGit }] },
]

function SkillLogo({ skill }: { skill: (typeof skillGroups)[number]['skills'][number] }) {
  const Icon = skill.icon
  return <span className="inline-flex items-center gap-2 border px-3 py-2 text-xs text-[var(--muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--text)]" title={skill.name}><Icon size={16} /><span className={mono}>{skill.name}</span></span>
}
      ...
      <div className={`max-w-[900px] border ${border}`}>
        ...
        <div className="space-y-12 p-[clamp(28px,5vw,60px)]">
          ...
          <section className="border-t pt-8" style={{ borderColor: 'var(--border)' }}>
            ...
            <div className="space-y-5">{skillGroups.map((group) => <div key={group.label}><p className={`mb-2 text-[9px] tracking-widest text-[var(--faint)] ${mono}`}>{group.label}</p><div className="flex flex-wrap gap-2">{group.skills.map((skill) => <SkillLogo key={skill.name} skill={skill} />)}</div></div>)}</div>
          </section>
          ...
          <section className="border-t pt-8" style={{ borderColor: 'var(--border)' }}>
            ...
            <div className="flex flex-wrap gap-3"><a href="https://github.com/sarbojitdutta" target="_blank" rel="noreferrer" aria-label="GitHub" className="inline-flex items-center gap-2 border px-4 py-2.5 text-xs text-[var(--muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--text)]"><FaGithub size={17} /> GitHub</a><a href="https://linkedin.com/in/sarbojitdutta" target="_blank" rel="noreferrer" aria-label="LinkedIn" className="inline-flex items-center gap-2 border px-4 py-2.5 text-xs text-[var(--muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--text)]"><FaLinkedin size={17} /> LinkedIn</a><a href="mailto:sarbojitd48@gmail.com" aria-label="Email" className="inline-flex items-center gap-2 border px-4 py-2.5 text-xs text-[var(--muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--text)]">Email</a></div>
          </section>
        </div>
      </div>
